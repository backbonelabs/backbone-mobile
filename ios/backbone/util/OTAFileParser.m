...
+ (void)parseFirmwareFileWithName:(NSString *)path onFinish:(void(^)(NSMutableDictionary *header, NSArray *rowData, NSArray *rowIdArray, NSError *error))finish {
  ...
  if (fileContents && fileContents.length > 0) {
    ...
    if (fileContentsArray) {
      fileContentsArray = [OTAFileParser removeEmptyRowsAndJunkDataFromArray:fileContentsArray];
      ...
      if (fileHeader.length >= FILE_HEADER_MAX_LENGTH) {
        ...
        for (NSString * dataRowString in fileContentsArray) {
          if (dataRowString.length > 20) {
            ...
            if ([OTAFileParser parseRowDataString:dataRowString] != nil) {
              [firmwareFileDataArray addObject:[OTAFileParser parseRowDataString:dataRowString]];
              ...
              error = [[NSError alloc] initWithDomain:@"ParsingError" code:FILE_PARSER_ERROR_CODE userInfo:[NSDictionary dictionaryWithObject:@"Invalid File" forKey:NSLocalizedDescriptionKey]];
              
              finish(nil,nil,nil, error);
            }
          }
          else {
            error = [[NSError alloc] initWithDomain:@"FileFormatError" code:FILE_PARSER_ERROR_CODE userInfo:[NSDictionary dictionaryWithObject:@"dataFormatInvalid" forKey:NSLocalizedDescriptionKey]];
            finish(nil,nil,nil, error);
            break;
          }
        }
        ...
        error = [[NSError alloc] initWithDomain:@"ParsingError" code:FILE_PARSER_ERROR_CODE userInfo:[NSDictionary dictionaryWithObject:@"invalidFile" forKey:NSLocalizedDescriptionKey]];
      ...
      error = [[NSError alloc] initWithDomain:@"ParsingError" code:FILE_PARSER_ERROR_CODE userInfo:[NSDictionary dictionaryWithObject:@"parsingFailed" forKey:NSLocalizedDescriptionKey]];
      
      finish(nil,nil,nil, error);
    }
  }
  else {
    error = [[NSError alloc] initWithDomain:@"FileEmpty" code:FILE_PARSER_ERROR_CODE userInfo:[NSDictionary dictionaryWithObject:@"fileEmpty" forKey:NSLocalizedDescriptionKey]];
    finish(nil,nil,nil, error);
  }
}
...
+ (NSMutableArray*)removeEmptyRowsAndJunkDataFromArray:(NSMutableArray*)dataArray {
  for (int i = 0; i < dataArray.count; ) {
    if ([[dataArray objectAtIndex:i] isEqualToString:@""]) {
      ...
      NSString *trimmedReplacement = [[[dataArray objectAtIndex:i] componentsSeparatedByCharactersInSet:charactersToRemove] componentsJoinedByString:@""];
